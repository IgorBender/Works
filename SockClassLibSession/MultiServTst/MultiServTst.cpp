// MultiServTst.cpp : Defines the entry point for the console application.
//
...
int main(int argc, char* argv[])
{
    ...
    SOCK_TRY
    {
        MulticastBound Serv(htons(15000), InterfAddr);
        ...
        Serv.leaveGroup(htonl(INADDR_ALLHOSTS_GROUP + 0x100));
#else

        JoinRes = Serv.leaveGroup(htonl(INADDR_ALLHOSTS_GROUP + 0x100));
        if(JoinRes == SOCKET_ERROR)
    {
        char Msg[256];
            SockException::formatMessage(&Msg);
