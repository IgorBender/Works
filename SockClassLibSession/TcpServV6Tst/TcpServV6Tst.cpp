...
int main(int argc, char* argv[])
{
    ...
    SOCK_TRY
    {
        ServerReuseV6 Serv(htons(SERV_PORT), InterfAddr);
        Serv.listen();
        ...
        while (!Exit)
        {
           shared_ptr < StreamSocketV6 > Sock(Serv.accept());
            ...
            cout << "Connection accepted : "
            		<< inet_ntop(AF_INET6, &SockName.sin6_addr, Tmp, 64) << "-" << ntohs(SockName.sin6_port)
            		<< " <--> "
            		<< inet_ntop(AF_INET6, &PeerName.sin6_addr, Tmp, 64) << "-" << ntohs(PeerName.sin6_port)
            		<< endl;
#endif
            SOCK_TRY
            {
