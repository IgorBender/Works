...
int main(int argc, char* argv[])
{
    ...
    SOCK_TRY
    {
        DatagramBoundIpDual Serv(htons(SERV_PORT));
        if(!Serv.isBound())
        {
            char Msg[256];
        ...
        forever
        {
            ...
            if(FD_ISSET(Serv.getSock(), &ReadSet))
			{
                ...
                {
#ifdef _MSC_VER
                    inet_ntop(AF_INET, From.sin6_addr.s6_addr + 12, FromAddrString,
                              INET6_ADDRSTRLEN);
#else
                    inet_ntop(AF_INET, &From.sin6_addr.s6_addr32[3], FromAddrString,
                              INET6_ADDRSTRLEN);
#endif
                }
                else
                {
                    ...
                    inet_ntop(AF_INET6, &From.sin6_addr, FromAddrString, INET6_ADDRSTRLEN);
                }

                cout << "Send to " << FromAddrString << endl;
