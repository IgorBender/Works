...
#ifndef SERVERSIMPLEIPDUAL_H
#define SERVERSIMPLEIPDUAL_H

#ifndef _NO_IPV6

#include "StreamBoundIpDual.h"
#include "StreamSocketV6.h"

/// TCP server socket.
class SOCKLIB_API ServerSimpleIpDual : public StreamBoundIpDual
{
public:
    /// Constructor.
    ...
    /// \throw SockException.
    ServerSimpleIpDual(uint16_t Port, in6_addr Address = in6addr_any,
                   bool LateBind = false) :
        StreamBoundIpDual(Port, Address, LateBind)
    {
    }
    /// Constructor.
    /// \param Port : port number for bind in network byte order.
    ...
    /// \param LateBind : do bind now or later according to bind method call.
    /// \throw SockException.
    ServerSimpleIpDual(uint16_t Port, const char* Address,
                       bool LateBind = false) :
        StreamBoundIpDual(Port, Address, LateBind)
    {
    }
...
private:
    ...
    ServerSimpleIpDual(ServerSimpleIpDual& s);
    /// Operator assign.
    /// Makes the class uncopyable.
    ServerSimpleIpDual& operator=(ServerSimpleIpDual& s);
};

#endif /* _NO_IPV6 */
#endif // SERVERSIMPLEIPDUAL_H
