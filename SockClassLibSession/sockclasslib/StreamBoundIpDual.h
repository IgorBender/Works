...
#ifndef STREAMBOUNDIPDUAL_H
#define STREAMBOUNDIPDUAL_H
...
public:
    ...
    /// \throw SockException.
    StreamBoundIpDual(uint16_t Port, in6_addr Address = in6addr_any,
                  bool LateBind = false);
    /// Constructor.
    /// \param Port : port number for bind in network byte order.
    ...
    /// \param LateBind : do bind now or later according to bind method call.
    /// \throw SockException.
    StreamBoundIpDual(uint16_t Port, const char* Address, bool LateBind = false);

private:
    /// Copy constructor.
    /// Makes the class uncopyable.
    StreamBoundIpDual(StreamBoundIpDual& s);
    /// Operator assign.
    /// Makes the class uncopyable.
    StreamBoundIpDual& operator=(StreamBoundIpDual& s);
};

#endif /* _NO_IPV6 */
#endif // STREAMBOUNDIPDUAL_H
