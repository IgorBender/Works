...
public:
    ...
    ServerSimple(short Port, in_addr_t Address = INADDR_ANY,
                 bool LateBind = false) throw(SockException) : StreamBoundV4(Port, Address, LateBind)
    {
    }
    ...
    /// \param LateBind : do bind now or later according to bind method call.
    /// \throw SockException.
    ServerSimple(short Port, const char* Address, bool LateBind = false)
    throw (SockException) : StreamBoundV4(Port, Address, LateBind)
    {
    }
#else

    ServerSimple(short Port, const char* Address, bool LateBind = false) :
    StreamBoundV4(Port, Address, LateBind)
    {
    }
#endif
    ...
    /// \throw SockException.
    virtual StreamSocketV4* accept() throw (SockException);
#else
