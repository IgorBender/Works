...
public:
    ...
    ServerSimpleV6(short Port, in6_addr Address = in6addr_any,
                 bool LateBind = false) throw(SockException) : StreamBoundV6(Port, Address, LateBind)
    {
    }
    ...
    /// \param LateBind : do bind now or later according to bind method call.
    /// \throw SockException.
    ServerSimpleV6(short Port, const char* Address, bool LateBind = false)
    throw (SockException) : StreamBoundV6(Port, Address, LateBind)
    {
    }
#else
    ServerSimpleV6(short Port, const char* Address, bool LateBind = false) :
    StreamBoundV6(Port, Address, LateBind)
    {
    }
#endif
    ...
    /// \throw SockException.
    virtual void listen(int Backlog = 5) throw (SockException);
    /// Accept connection request.
    /// \return StreamSocketV6 pointer representing local connection endpoint.
    /// \throw SockException.
    virtual StreamSocketV6* accept() throw (SockException);
#else
