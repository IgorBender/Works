...
#ifndef _CONNECTEDSOCKETV4_H
#define _CONNECTEDSOCKETV4_H
...
public:
    ...
    /// \param Port : port number in network byte order.
    /// \throw SockException.
    void connect(in_addr_t Address, short Port) throw(SockException)
    {
    ...
    /// \param Port : port number in network byte order.
    /// \throw SockException.
    void connect(const char* Address, short Port) throw(SockException)
    {
        setDestination(Address, Port);
        connect();
    }
    ...
    /// \param Dest : sockaddr_in structure filled with destination endpoint properties.
    /// \throw SockException.
    void connect(sockaddr_in Dest) throw(SockException)
    {
        setDestination(Dest);
        connect();
    }
