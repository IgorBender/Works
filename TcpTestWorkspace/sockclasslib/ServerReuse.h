...
class SOCKLIB_API ServerReuse : public ServerSimple
{
public:
    ...
    /// \throw SockException.
    ServerReuse(short Port, in_addr_t Address = INADDR_ANY) throw (SockException);
    /// Constructor.
    /// \param Port : port number for bind in network byte order.
    /// \param Address : IPv4 address for bind in decimal dot notation.
    /// \throw SockException.
    ServerReuse(short Port, const char* Address) throw (SockException);
#else
