...
public:
    ...
    /// \throw SockException.
    StreamBoundV6(short Port, in6_addr Address = in6addr_any, bool LateBind = false) throw (SockException);
    /// Constructor.
    /// \param Port : port number for bind in network byte order.
    /// \param Address : IPv6 address for bind in textual notation.
    /// \param LateBind : do bind now or later according to bind method call.
    /// \throw SockException.
    StreamBoundV6(short Port, const char* Address, bool LateBind = false) throw (SockException);
#else
